  ...
  save(cb) {
    ...
    connection.query(`INSERT INTO comments (comment_text, user_id, photo_id) VALUES ('${comment.comment_text}', '${comment.user_id}', '${comment.photo_id}' )`,
      function(err, results) {
        if (err) throw err;
        connection.query(`SELECT LAST_INSERT_ID() AS cid`,
          function(err, results) {
            if (err) throw err;
            console.log(results)
...
Comment.findByCommentId = function(cid, cb) {
  connection.query(`SELECT * FROM comments WHERE id='${cid}'`, function(err, results) {
    if (err) {
      cb(err);
      return;
    }
